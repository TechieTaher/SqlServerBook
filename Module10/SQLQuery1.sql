...
SELECT dbo.FormatPhoneNumber('123456789');


Create FUNCTION dbo.IntegerListToTable 
( @fullName nvarchar(30), @delimiter nvarchar(1))
RETURNS @result TABLE (firstName nvarchar(100))
AS BEGIN
	INSERT INTO @result (firstName)
	SELECT value from STRING_SPLIT ( @fullName , @delimiter );
	RETURN;
END;

select * from dbo.IntegerListToTable ('John Doe',' ')
